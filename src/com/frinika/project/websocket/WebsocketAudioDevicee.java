...
import com.frinika.project.websocket.WebsocketAudioDeviceProvider.WebsocketAudioDeviceeInfo;
...
public class WebsocketAudioDevicee extends AbstractMixer {
  ...
  // CONSTRUCTOR
  WebsocketAudioDevicee(WebsocketAudioDeviceProvider.WebsocketAudioDeviceeInfo portMixerInfo) {
  ...
  private DirectDLI createDataLineInfo(boolean isSource) {
    AudioFormat defaultAAudioFormat =  new AudioFormat(Encoding.PCM_SIGNED,44100,16,2,4,44100,false);
    AudioFormat[] deAudioFormats =new AudioFormat[1];
    Arrays.asList(defaultAAudioFormat).toArray(deAudioFormats);
      return new DirectDLI(isSource? SourceDataLine.class: TargetDataLine.class,
          deAudioFormats, deAudioFormats,
          32, // arbitrary minimum buffer size
          AudioSystem.NOT_SPECIFIED);
  }
...
//        }
//      }
System.out.println("requested;"+dataLineInfo.toString());
      if (dataLineInfo.getLineClass().equals(DirectSDL.class)) {
        return new DirectSDL(dataLineInfo, dataLineInfo.getFormats()[0], DEFAULT_LINE_BUFFER_TIME, this);
      }
      if (dataLineInfo.getLineClass().equals(DirectClip.class)) {
        return new DirectClip(dataLineInfo, dataLineInfo.getFormats()[0], CLIP_BUFFER_TIME, this);
      }
      if (dataLineInfo.getLineClass().equals(DirectTDL.class)) {
        return new DirectTDL(dataLineInfo, dataLineInfo.getFormats()[0], DEFAULT_LINE_BUFFER_TIME, this);
      }
//    }
    throw new IllegalArgumentException("Line unsupported: " + info);
  }
  ...
  int getMixerIndex() {
    return ((WebsocketAudioDeviceeInfo) getMixerInfo()).getIndex();
  }

  int getDeviceID() {
    return ((WebsocketAudioDeviceeInfo) getMixerInfo()).getDeviceID();
  }

  int getMaxSimulLines() {
    return ((WebsocketAudioDeviceeInfo) getMixerInfo()).getMaxSimulLines();
  }
   ...
   */
  protected static final class DirectDLI extends DataLine.Info {
   ...
   */
  protected static class DirectDL extends AbstractDataLine implements EventDispatcher.LineMonitor {
...
//      }
   ...
   */
  protected static final class DirectSDL extends DirectDL
   ...
   */
  protected static final class DirectTDL extends DirectDL
