...
public class WebsocketMainMixer {
  ...
  private final WebsocketMixer mixer;
  ...
  private final List<WebsocketSourceDataLine> openLinesList = new ArrayList<WebsocketSourceDataLine>();

  private WebsocketSourceDataLine[] openLines = new WebsocketSourceDataLine[0];
  ...
  void processAudioBuffers() {
    ...
    WebsocketSourceDataLine[] openLines;
  ...
  public WebsocketMainMixer(WebsocketMixer mixer) {
    this.mixer = mixer;
    ...
    InputStream in = new InputStream() {
      ...
      private final SoftAudioBuffer[] buffers = WebsocketMainMixer.this.buffers;

      private final int nrofchannels = WebsocketMainMixer.this.mixer.getFormat().getChannels();

      private final int buffersize = buffers[0].getSize();

      private final byte[] bbuffer = new byte[buffersize
          * (WebsocketMainMixer.this.mixer.getFormat()
      ...
      public void close() {
        WebsocketMainMixer.this.mixer.close();
      }
  ...
  public void openLine(WebsocketSourceDataLine line) {
    synchronized (control_mutex) {
      openLinesList.add(line);
      openLines = openLinesList
          .toArray(new WebsocketSourceDataLine[openLinesList.size()]);
    }
  }

  public void closeLine(WebsocketSourceDataLine line) {
    synchronized (control_mutex) {
  ...
  public WebsocketSourceDataLine[] getOpenLines() {
    synchronized (control_mutex) {
      return openLines;
    }
  ...
  public void close() {
    WebsocketSourceDataLine[] openLines = this.openLines;
    for (int i = 0; i < openLines.length; i++) {
      openLines[i].close();
    }
  }
